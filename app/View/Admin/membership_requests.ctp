...
<script type="text/javascript">
	$(function() {
		...
		$('body').on('click', '[data-approve]', function() {
			...
			$.ajax({
				url: '<?php echo $this->webroot; ?>admin/ajax_approve_membership_request',
				type: 'POST',
				data: params,
				beforeSend: function() {
				...
				success: function (result) {
					...
					try {
						...
						if(result['success']) {
							...
							show_alerts({alerts: get_alert('error', 'There are not enough memberships. Generate memberships first.'), anchor: $('#content'), insert_type: 'prepend'});
						}
					}
					catch(e) {
		...
		$('body').on('click', '[data-deny]', function() {
			...
			if(confirm('Are you sure you want to deny this membership request?')) {
				$.ajax({
					url: '<?php echo $this->webroot; ?>admin/ajax_deny_membership_request',
					type: 'POST',
					data: params,
					beforeSend: function() {
						$('#requests-container').append(ajax_loader);
		...
		function get_requests(params) {
			$.ajax({
				url: '<?php echo $this->webroot; ?>admin/ajax_get_membership_requests',
				type: 'POST',
				data: params,
				beforeSend: function() {
					$('#requests-container').append(ajax_loader);
