	...
	class PagesController extends AppController {
		...
		private function network() {
			if(!$this->Auth->User()) {
				...
				$request = $this->Request->find('first', array('conditions' => array('Request.purpose' => 'join_network', 'Request.user_id' => $this->Auth->User('id'))));
		...
		public function ajax_join_network() {
			$allowed = array('pin', 'username');
			$params = $this->uniform_params($this->request->data, $allowed);

			$this->User->recursive = 0;
			$user = $this->User->find('first', array('User.username' => $params['username']));
			if($user) {
				if(!$user['User']['activated']) {
					$this->Session->setFlash('The sponsor you entered is not a valid member.', 'error');
					$this->redirect('/network');
				}
				elseif (count($user['User']) == 17) {
					$this->Session->setFlash('The sponsor you entered cannot have more direct members.', 'error');
					$this->redirect('/network');
				}
				$params['amount'] = $user['User']['id'];
			...
			if($epin) {
				...
				elseif ($epin['Epin']['user_id'] != 0)
					...
					$this->Session->setFlash('Request to join network is successfull.'.count($user['User']), 'success');
				}
				$this->redirect('/network');
			}
			else {
